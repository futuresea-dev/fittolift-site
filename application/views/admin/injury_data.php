<div class="right_col" role="main">
          <div class="">
            <div class="page-title">
              <div class="title_left">
<!--                <h3>Reservations</h3>-->
                  
              </div>

<!--
              <div class="title_right">
                <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
                  <div class="input-group">
                    <input type="text" class="form-control" placeholder="Search for...">
                    <span class="input-group-btn">
                      <button class="btn btn-default" type="button">Go!</button>
                    </span>
                  </div>
                </div>
              </div>
-->
            </div>

            <div class="clearfix"></div>

            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                  
                <?php if($this->session->flashdata('success')){ ?>
                <div class="alert alert-success" style="font-size: 14px;padding: 10px;">
                    <strong>Success! </strong><?php echo $this->session->flashdata('success')?> 
                </div>
                <?php }if($this->session->flashdata('error')){ ?>
                    <div class="alert alert-danger" style="font-size: 13px;padding: 10px; text-shadow: none;">
                      <strong>Error! </strong> <?php echo $this->session->flashdata('error')?>
                    </div>
                <?php } ?>
                  
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Injury Form Data <small></small></h2>
<!--
                    <ul class="nav navbar-right panel_toolbox">
                      <li><a class="collapse-link"><i class="fa fa-chevron-up"></i></a>
                      </li>
                      <li class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false"><i class="fa fa-wrench"></i></a>
                        <ul class="dropdown-menu" role="menu">
                          <li><a href="#">Settings 1</a>
                          </li>
                          <li><a href="#">Settings 2</a>
                          </li>
                        </ul>
                      </li>
                      <li><a class="close-link"><i class="fa fa-close"></i></a>
                      </li>
                    </ul>
-->
                    <div class="clearfix"></div>
                  </div>
                  <div class="x_content">
                    
                    <table id="datatable" class="table table-striped table-bordered">
                      <thead>
                        <tr>
                            <th>ID</th>   
                            <th>Employer</th>
                            <th>Clinic</th>
<!--                            <th>DOB</th>        -->
                            <th>Injury Date</th>
                            <th>Injury Nature</th>
                            <th>Doctor's Name</th>
                            <th>Submission Date</th>    
                            <th>Action</th>
                        </tr>
                      </thead>


                      <tbody>
                        
                        <?php
                        foreach($injury_data as $res){
                            //echo '<pre>';print_r($res);
                            $emp_data = $this->Admin_model->get_assigned_clinic_data($res['employer_id']);
                                
                        ?>
                          
  <div class="modal-content" id="pdf_data_<?php echo $res['id'];?>" style="display:none; width:100%;font-size:15px;color:black;">

        <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel" style="margin:bottom:15px;font-size: 24px;float: left;color:black;">General Employer : Clinical Injury Report</h4>
            <hr>
            <img src="<?php echo base_url().'assets/images/clinic_logo.png';?>" style="width: 130px; float: right; margin-right: 70px;">
    

        <div style="margin-top:25px;">  Employer Name  .........................  <?php echo $emp_data[0]['username'];?></div>    
        <div>  Clinic Name                      .........................  <?php echo $res['clinic_name'];?> </div>
        <div>  Date of Birth                    .........................  <?php echo date("d M, Y", strtotime($res['dob']));?></div>    
        <div>  First Name                       .........................  <?php echo $res['first_name'];?></div>
        <div>  Last Name                        .........................  <?php echo $res['last_name'];?></div>    
        <div>  Employee Number                  .........................  <?php echo $res['employee_no'];?></div>
        <div>  Employer Location                .........................  <?php echo wordwrap($res['emp_location'], 50, "<br />\n");?> </div>
        <div>  Gender                           .........................  <?php echo $res['gender'];?></div>
        <div>  Type of Office Visit             .........................  <?php echo $res['office_visit'];?></div>    
        <div>  Is physcially Fit for Duty       .........................  <?php echo wordwrap($res['duty'], 45, "<br />\n");?></div>
        <div>  Drug Test                        .........................  <?php echo $res['drug_test'];?> </div>
        <div>  Expected Return Date             .........................  <?php echo date("d M, Y", strtotime($res['return_date']));?></div>
        <div>  Date of next evaluation          .........................  <?php echo date("d M, Y", strtotime($res['next_eval']));?></div>
        <div>  Department Name (Customize)      .........................  <?php echo $res['dept_name'];?></div> 
        <div>  Date of Injury Loss/Incident    ..........................  <?php echo date("d M, Y", strtotime($res['injury_date']));?></div>
        <div>  Time of Day                      .........................  <?php echo $res['time_of_day'];?> </div>
        <div>  Day of Week                      .........................  <?php echo $res['loss_day'];?></div>    
        <div>  Date reported to Office          .........................  <?php echo date("d M, Y", strtotime($res['date_dc']));?> </div>
        <div>  Were the 1st provider seen?      .........................  <?php echo $res['provider_seen'];?> </div>
        <div>  Was Treatment Required?          .........................  <?php echo $res['treatment'];?></div>
        <div>  Was First Aid Required?          .........................  <?php echo $res['first_aid'];?></div>    
        <div>  Nature of the Injury             .........................  <?php echo $res['injury_nature'];?></div>
        <div>  Cause of the Injury              .........................  <?php echo $res['injury_cause'];?></div>
        <div>  Area of body Injured             .........................  <?php echo $res['area_injured'];?> </div>
        <div>  Alleged Incident Description     .........................  <?php echo wordwrap($res['inc_desc'], 50, "<br />\n");?> </div>
<!--
        <div>  Limitations Arm Lift</span> <span id="lim_arm" class="span_lable_2"><?php echo $res['arm_lift'];?> </span> </div>
        <div>  Limitations High Near Lift</span> <span id="lim_high_near" class="span_lable_2"><?php echo $res['near_left'];?> </span> </div>
        <div>  Limitations High Far Lift</span> <span id="lim_high_far" class="span_lable_2"><?php echo $res['far_lift'];?> </span> </div>
--> 
        <div>  Lift from the waist up to        ...................................  <?php echo $res['waist_lift'];?></div>    
        <div>  Lift at shoulder level up to     ...................................  <?php echo $res['shoulder_lift'];?> </div>
        <div>  Lift above shoulder level up to  ...................................  <?php echo $res['shoulder_level'];?> </div>
        <div>  Lift from the knee level up to   ...................................  <?php echo $res['knee_lift'];?> </div>
        <div>  Lift from the floor level up to  ...................................  <?php echo $res['floor_lift'];?> </div>
        <div>  Sitting and standing             ...................................  <?php echo wordwrap($res['sitting'], 50, "<br />\n");?> </div>
        <div>  Climbing                         ...................................  <?php echo $res['climbing'];?> </div>    
        <div>  Written Job description          ...................................  <?php echo wordwrap($res['job_desc'], 50, "<br />\n");?> </div>
        <div>  Department Number                ...................................  <?php echo $res['dept_no'];?> </div>
        <div>  Department Manager               ...................................  <?php echo $res['dept_manager'];?></div>    
        <div>  Department Name                  ...................................  <?php echo $res['dept_name'];?> </div>    
        <div>  Evaluating doctor's name         ...................................  <?php echo $res['doctor_name'];?></div>
        <div>  Form Submission Date             ...................................  <?php echo date("d M, Y", strtotime($res['submission']));?> </div>     


        </div>


      </div>
                          
                          
                          
                        <div class="modal fade bs-example-modal-lg-<?php echo $res['id'];?>" tabindex="-1" role="dialog" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                              <div class="modal-content">

                                <div class="modal-header">
                                  <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span>
                                  </button>
                                    <h4 class="modal-title" id="myModalLabel" style="font-size: 24px;float: left;">General Employer Injury Report</h4>
                                    <img src="<?php echo base_url().'assets/images/clinic_logo.png';?>" style="width: 130px; float: right; margin-right: 40px;">
                                </div>
                                 
                                <div class="modal-body" id="content" style="text-align:center;  font-size: 14px;">
                                
                                <div> <span class="span_lable">Employer Name</span> <span id="emp_id" class="span_lable_2"><?php echo $emp_data[0]['username'];?> </span> </div>    
                                <div> <span class="span_lable">Clinic Name</span> <span id="emp_no" class="span_lable_2"><?php echo $res['clinic_name'];?> </span> </div>
                                <div> <span class="span_lable">Date of Birth</span> <span id="exp_return" class="span_lable_2"><?php echo date("d M, Y", strtotime($res['dob']));?> </span> </div>    
                                <div> <span class="span_lable">First Name</span> <span id="is_fit" class="span_lable_2"><?php echo $res['first_name'];?> </span> </div>
                                <div> <span class="span_lable">Last Name</span> <span id="is_fit" class="span_lable_2"><?php echo $res['last_name'];?> </span> </div>    
                                <div> <span class="span_lable">Employee Number</span> <span id="is_fit" class="span_lable_2"><?php echo $res['employee_no'];?> </span> </div>
                                <div> <span class="span_lable">Location</span> <span id="is_fit" class="span_lable_2"><?php echo $res['emp_location'];?> </span> </div>
                                <div> <span class="span_lable">Gender</span> <span id="is_fit" class="span_lable_2"><?php echo $res['gender'];?> </span> </div>
                                <div> <span class="span_lable">Type of Office Visit</span> <span id="is_fit" class="span_lable_2"><?php echo $res['office_visit'];?> </span> </div>    
                                <div> <span class="span_lable">Is physcially Fit for Duty</span> <span id="is_fit" class="span_lable_2"><?php echo $res['duty'];?> </span> </div>
                                <div> <span class="span_lable">Drug Test</span> <span id="is_fit" class="span_lable_2"><?php echo $res['drug_test'];?> </span> </div>
                                <div> <span class="span_lable">Expected return for work date</span> <span id="exp_return" class="span_lable_2"><?php echo date("d M, Y", strtotime($res['return_date']));?> </span> </div>
                                <div> <span class="span_lable">Date of next evaluation</span> <span id="exp_next_eve" class="span_lable_2"><?php echo date("d M, Y", strtotime($res['next_eval']));?> </span> </div>
                                <div> <span class="span_lable">Department Name</span> <span id="dep_name" class="span_lable_2"><?php echo $res['dept_name'];?> </span> </div> 
                                <div> <span class="span_lable">Date of Injury Loss/Incident</span> <span id="inj_date" class="span_lable_2"><?php echo date("d M, Y", strtotime($res['injury_date']));?> </span> </div>
                                <div> <span class="span_lable">Time of Day</span> <span id="time_day" class="span_lable_2"><?php echo $res['time_of_day'];?> </span> </div>
                                <div> <span class="span_lable">Day of Week</span> <span id="time_day" class="span_lable_2"><?php echo $res['loss_day'];?> </span> </div>    
                                <div> <span class="span_lable">Date reported to your (DC) office</span> <span id="date_reported_off" class="span_lable_2"><?php echo date("d M, Y", strtotime($res['date_dc']));?> </span></div>
                                <div> <span class="span_lable">Were you the 1st provider seen?</span> <span id="were_seen" class="span_lable_2"><?php echo $res['provider_seen'];?> </span> </div>
                                <div> <span class="span_lable">Was Treatment Required?</span> <span id="were_seen" class="span_lable_2"><?php echo $res['treatment'];?> </span> </div>
                                <div> <span class="span_lable">Was First Aid Required?</span> <span id="were_seen" class="span_lable_2"><?php echo $res['first_aid'];?> </span> </div>    
                                <div> <span class="span_lable">Nature of the Injury</span> <span id="ing_nature" class="span_lable_2"><?php echo $res['injury_nature'];?> </span> </div>
                                <div> <span class="span_lable">Cause of the Injury</span> <span id="ing_cause" class="span_lable_2"><?php echo $res['injury_cause'];?> </span> </div>
                                <div> <span class="span_lable">Area of body Injured</span> <span id="inj_area" class="span_lable_2"><?php echo $res['area_injured'];?> </span> </div>
                                <div> <span class="span_lable">Alleged Incident Description</span> <span id="inc_desc" class="span_lable_2"><?php echo $res['inc_desc'];?> </span> </div>
<!--
                                <div> <span class="span_lable">Limitations Arm Lift</span> <span id="lim_arm" class="span_lable_2"><?php echo $res['arm_lift'];?> </span> </div>
                                <div> <span class="span_lable">Limitations High Near Lift</span> <span id="lim_high_near" class="span_lable_2"><?php echo $res['near_left'];?> </span> </div>
                                <div> <span class="span_lable">Limitations High Far Lift</span> <span id="lim_high_far" class="span_lable_2"><?php echo $res['far_lift'];?> </span> </div>
--> 
                                <div> <span class="span_lable">Lift from the waist up to</span> <span id="lim_leg" class="span_lable_2"><?php echo $res['waist_lift'];?> </span> </div>    
                                <div> <span class="span_lable">Lift at shoulder level up to</span> <span id="lim_leg" class="span_lable_2"><?php echo $res['shoulder_lift'];?> </span> </div>
                                <div> <span class="span_lable">Lift above shoulder level up to</span> <span id="lim_floor" class="span_lable_2"><?php echo $res['shoulder_level'];?> </span> </div>
                                <div> <span class="span_lable">Lift from the knee level up to</span> <span id="lim_sitting" class="span_lable_2"><?php echo $res['knee_lift'];?> </span> </div>
                                <div> <span class="span_lable">Lift from the floor level up to</span> <span id="lim_sitting" class="span_lable_2"><?php echo $res['floor_lift'];?> </span> </div>
                                <div> <span class="span_lable">Sitting and standing</span> <span id="lim_sitting" class="span_lable_2"><?php echo $res['sitting'];?> </span> </div>
                                <div> <span class="span_lable">Climbing</span> <span id="lim_sitting" class="span_lable_2"><?php echo $res['climbing'];?> </span> </div>    
                                <div> <span class="span_lable">Written Job description reviewed</span> <span id="written_job_des" class="span_lable_2"><?php echo $res['job_desc'];?> </span> </div>
                                <div> <span class="span_lable">Department Number</span> <span id="doctor_name" class="span_lable_2"><?php echo $res['dept_no'];?> </span> </div>
                                <div> <span class="span_lable">Department Manager</span> <span id="doctor_name" class="span_lable_2"><?php echo $res['dept_manager'];?> </span> </div>    
                                <div> <span class="span_lable">Department Name</span> <span id="doctor_name" class="span_lable_2"><?php echo $res['dept_name'];?> </span> </div>    
                                <div> <span class="span_lable">Evaluating doctor's name</span> <span id="doctor_name" class="span_lable_2"><?php echo $res['doctor_name'];?> </span> </div>    
                                    
                                    
                                </div>
                                
                                <div class="modal-footer">
                                  <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                   <a href="javascript:demoFromHTML(<?php echo $res['id'];?>)" class="btn btn-primary">Download PDF</a>
                                    
                                </div>
                        

                              </div>
                            </div>
                        </div>  
                          
                        <tr>
                            <td><?php echo $res['id'];?></td>
                            <td><?php echo $emp_data[0]['username'];?></td>
                            <td><?php echo $res['clinic_name'];?></td>
<!--                            <td><?php echo date("d M, Y", strtotime($res['birth_year']));?></td>-->
                            <td><?php echo date("d M, Y", strtotime($res['injury_date']));?></td>
                            <td><?php echo $res['injury_nature'];?></td>
                            <td><?php echo $res['doctor_name'];?></td>
                            <td><?php echo date("d M, Y", strtotime($res['submission']));?></td>    
                            
                            <td>
<!--
                                <a href="<?php echo base_url("admin/profile/".$res['id']); ?>" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> View </a>
                                <a href="<?php echo base_url("admin/editUser/".$res['id']); ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>
-->
                                <?php if($this->session->userdata('user_type') == 'admin' ){ ?>
                                <a onclick="javascript:deleteConfirm('<?php echo base_url("admin/delInjurydata/".$res['id']);?>');" deleteConfirm href="#" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a>
                                <?php } ?>
                                
                                <a href="#" class="btn btn-success btn-xs" data-toggle="modal" data-target=".bs-example-modal-lg-<?php echo $res['id'];?>"><i class="fa fa-user-plus"></i> Injury Report </a>
                            </td>

                        </tr>
                        <?php }?>
                          
                      </tbody>
                    </table>
                  </div>
                </div>
              </div>

              </div>
          </div>
        </div>
        <!-- /page content -->



    <?php include('includes/footer.php');?>


 <script>
    function demoFromHTML(id) {
       var imgData = 'data:image/jpeg;base64,'
        
        
        
        var pdf = new jsPDF('p', 'pt', 'letter');
        
        pdf.addImage(imgData, 'JPEG', 240,30,150,30);
        //pdf.text(210, 60, 'Employee Injury Report');
        //pdf.text(100, 70, '---------------------------------------------------------------------------');
        // source can be HTML-formatted string, or a reference
        // to an actual DOM element from which the text will be scraped.
        source = $('#pdf_data_'+id)[0];

        // we support special element handlers. Register them with jQuery-style 
        // ID selector for either ID or node name. ("#iAmID", "div", "span" etc.)
        // There is no support for any other type of selectors 
        // (class, of compound) at this time.
        specialElementHandlers = {
            // element with id of "bypass" - jQuery style selector
            '#bypassme': function (element, renderer) {
                // true = "handled elsewhere, bypass text extraction"
                return true
            }
        };
        margins = {
            top: 60,
            bottom: 40,
            left: 110,
            width: 2000
        };
        // all coords and widths are in jsPDF instance's declared units
        // 'inches' in this case
        pdf.fromHTML(
            source, // HTML string or DOM elem ref.
            margins.left, // x coord
            margins.top, { // y coord
                'width': margins.width, // max width of content on PDF
                'elementHandlers': specialElementHandlers
            },

            function (dispose) {
                // dispose: object with X, Y of the last line add to the PDF 
                //          this allow the insertion of new lines after html
                pdf.save('injury-report.pdf');
            }, margins
        );
    }
</script>

    <script type="text/javascript"> 
        function deleteConfirm(url)
         {
            if(confirm('Do you want to Delete this Injury Data ?'))
            {
                window.location.href=url;
            }
         }
    </script>
    <!-- <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/1.3.2/jspdf.min.js"></script>-->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/1.5.3/jspdf.debug.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/1.5.3/jspdf.min.js"></script>
    <!-- Datatables -->
    <script>
      $(document).ready(function() {
      

//        var doc = new jsPDF();
//        var specialElementHandlers = {
//            '#editor': function (element, renderer) {
//                return true;
//            }
//        };
//
//        $('#cmds').click(function () {
//            $emp_id = $('#emp_id').html();
//            $emp_no = $('#emp_no').html();
//            $dob = $('#dob').html();
//            $is_fit = $('#is_fit').html();
//            $exp_return = $('#exp_return').html();
//            
//            $exp_next_eve = $('#exp_next_eve').html();
//            $dep_name = $('#dep_name').html();
//            $inj_date = $('#inj_date').html();
//            $time_day = $('#time_day').html();
//            $date_reported_off = $('#date_reported_off').html();
//            $were_seen = $('#were_seen').html();
//            $ing_nature = $('#ing_nature').html();
//            $ing_cause = $('#ing_cause').html();
//            $inj_area = $('#inj_area').html();
//            $inc_desc = $('#inc_desc').html();
//            
//            $lim_arm = $('#lim_arm').html();
//            $lim_high_near = $('#lim_high_near').html();
//            $lim_high_far = $('#lim_high_far').html();
//            $lim_leg = $('#lim_leg').html();
//            $lim_floor = $('#lim_floor').html();
//            $lim_sitting = $('#lim_sitting').html();
//            $written_job_des = $('#written_job_des').html();
//            $doctor_name = $('#doctor_name').html();
//            
//            doc.text(75, 15, 'Employee Injury Report:');
//            
//            doc.text(30, 30, 'Employer Name :                        '+$emp_id);
//            doc.text(30, 40, 'Clinic Name :                              '+$emp_no);
//            doc.text(30, 50, 'Date of Birth :                             '+$dob);
//            doc.text(30, 60, 'Is the Employee Fit for Duty :     '+$is_fit);
//            doc.text(30, 70, 'Expected return for work date :   '+$exp_return);
//            doc.text(30, 80, 'Date of next evaluation :              '+$exp_next_eve);
//            doc.text(30, 90, 'Department Name (Customize) :    '+$dep_name);
//            doc.text(30, 100, 'Date of Injury Loss/Incident :        '+$inj_date);
//            doc.text(30, 110, 'Time of Day :                                  '+$time_day);
//            doc.text(30, 120, 'Date reported to your (DC) office :  '+$date_reported_off);
//            doc.text(30, 130, 'Were you the 1st provider seen? :   '+$were_seen);
//            doc.text(30, 140, 'Nature of the Injury :                   '+$ing_nature);
//            doc.text(30, 150, 'Cause of the Injury :                   '+$ing_cause);
//            doc.text(30, 160, 'Area of body Injured :                 '+$inj_area);
//            doc.text(30, 170, 'Alleged Incident Description :     '+$inc_desc);
//            doc.text(30, 180, 'Limitations Arm Lift :                 '+$lim_arm);
//            
//            doc.text(30, 190, 'Limitations High Near Lift :        '+$lim_high_near);
//            doc.text(30, 200, 'Limitations High Far Lift :          '+$lim_high_far);
//            doc.text(30, 210, 'Limitations Leg Lift :                  '+$lim_leg);
//            doc.text(30, 220, 'Limitations Floor Lift :                '+$lim_floor);
//            doc.text(30, 230, 'Limitations to Sitting :                '+$lim_sitting);
//            doc.text(30, 240, 'Written Job description reviewed (Customizable) :   '+$written_job_des);
//            doc.text(30, 250, "Evaluating doctor's name :          "+$doctor_name);
////            doc.fromHTML($('#content').html(), 40, 20, {
////                'width': 700,
////                    'elementHandlers': specialElementHandlers
////            });
//            doc.save('injury-report.pdf');
//        });  
//          
      });
    </script>
    <!-- /Datatables -->
    <script>tinymce.init({ selector:'textarea', menubar: false,height: 250, statusbar : false, force_p_newlines: false });</script>
    <style>
        .span_lable{width: 250px; display: inline-block;font-weight: bold; text-align: left;float: left;}
        .span_lable_2{width: 275px; display: inline-block;font-weight: bold; text-align: left; color: cornflowerblue;}    
    </style>

